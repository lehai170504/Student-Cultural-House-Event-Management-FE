...
const users: User[] = [
  { id: 1, name: "Nguyễn Văn A", email: "a@student.edu.vn", role: "student", status: "active" },
  { id: 2, name: "Trần Thị B", email: "b@org.vn", role: "org", status: "inactive" },
  { id: 3, name: "Lê Văn C", email: "c@booth.vn", role: "booth", status: "active" },
];
...
export default function UserManagement() {
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      <section className="relative bg-white rounded-2xl shadow p-8 mt-5">
        <div className="container mx-auto px-6">
          ...
          <div className="grid md:grid-cols-2 gap-6 items-center mb-8">
            <div>
              <h1 className="text-3xl font-bold text-gray-800 mb-2">Quản lý tài khoản người dùng</h1>
              <p className="text-lg text-gray-600">Bao gồm Sinh viên, Ban tổ chức và Gian hàng</p>
            </div>

            <div className="flex md:justify-end justify-center gap-4 flex-wrap items-center">
              <div className="flex items-center gap-3">
                <Label htmlFor="role" className="text-gray-700 font-medium">Lọc theo vai trò:</Label>
              ...
              <TableBody>
                {filteredUsers.map((user) => (
                  <TableRow key={user.id}>
                    ...
                    <TableCell className="px-6 py-4">{roleLabels[user.role]}</TableCell>
                    <TableCell className="px-6 py-4">
                      <span
                        className={`px-3 py-1 rounded-full text-sm ${statusLabels[user.status].bg} ${statusLabels[user.status].text}`}
                      >
      ...
      {/* Popup */}
      <CreateOrEditUser open={openModal} onClose={() => setOpenModal(false)} user={selectedUser} />
    </main>
  );
}
